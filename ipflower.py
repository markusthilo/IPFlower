...
IPFlower provides a CSV list or HTML/JS network visualisation from network analysis generated by Zeek (conn.log) or iprunner.
...
''')
	...
	argparser.add_argument('-c', '--config', type=str,
		help='Cofiguration File (default: ~/.config/ipflower.conf)', metavar='FILE'
	)
	...
	argparser.add_argument('-i', '--reverse', action='store_true',
		help='Reverse order on CSV output'
	)
	...
	argparser.add_argument('-p', '--ports', action='store_true',
		help='Works with Zeek conn.log: differentiate by server ports (id.resp_p)'
	)
	argparser.add_argument('-t', '--type', dest='datatype',
		help='Type of input file(s) and calculation - use -t l or -t list supported types', metavar='STRING'
	)
	...
	argparser.add_argument('-v', '--vis', action='store_true',
		help='Generate HTML/JavaScript for VISJS (default is CSV output)'
	)
...
''')
	...
	if args.infiles == []:
		...
		SysExit(1)
	if args.datatype == None or args.datatype.lower() in ('z', 'zeek'):
		stats = Zeek(
			args.infiles,
			ports = args.ports,
			grep=args.grep,
